 ...
 * Browser-compatible version adapted from SVGO.
 */

import { removeLeadingZero, toFixed } from './tools';
import type { PathDataItem, PathDataCommand, StringifyPathDataOptions } from './types';

type ReadNumberState = 'none' | 'sign' | 'whole' | 'decimal_point' | 'decimal' | 'e' | 'exponent_sign' | 'exponent';
...
const roundAndStringify = (number: number, precision?: number): { roundedStr: string, rounded: number } => {
  if (precision != null) {
