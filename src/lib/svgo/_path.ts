/**
 * Browser-compatible version adapted from SVGO plugins/_path.js
 */
import { parsePathData, stringifyPathData } from './path';
import type { PathDataItem, XastElement } from './types';

interface Js2PathParams {
 ...
 */
export const path2js = (path: XastElement & { pathJS?: PathDataItem[] }): PathDataItem[] => {
  if (path.pathJS) {
    return path.pathJS;
  }
 ...
 */
const convertRelativeToAbsolute = (data: ReadonlyArray<PathDataItem>): PathDataItem[] => {
...
export const js2path = function (
  path: XastElement & { pathJS?: PathDataItem[] },
  data: ReadonlyArray<PathDataItem>,
 ...
 */
export const intersects = function (path1: ReadonlyArray<PathDataItem>, path2: ReadonlyArray<PathDataItem>): boolean {
  ...
  function getSupport(a: Point, b: Point, direction: ReadonlyArray<number>): number[] {
   ...
   */
  function supportPoint(polygon: Point, direction: ReadonlyArray<number>): number[] {
    let index =
      direction[1] >= 0
        ? direction[0] < 0
          ? polygon.maxY
...
function gatherPoints(pathData: ReadonlyArray<PathDataItem>): Points {
   ...
   */
  const addPoint = (path: Point, point: number[]): void => {
  ...
  for (let i = 0; i < pathData.length; i += 1) {
    ...
    const toAbsolute = (n: number, i: number): number => n + (basePoint == null ? 0 : basePoint[i % 2]);
